...
static void TestPackageSelection(const CChainParams& chainparams, const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst) EXCLUSIVE_LOCKS_REQUIRED(cs_main, ::mempool.cs)
{
    ...
    std::unique_ptr<CBlockTemplate> pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,CKeyID(),0,0,MakeTransactionRef());
    ...
    pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,CKeyID(),0,0,MakeTransactionRef());
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    std::string strAddress = "1128xFWM8C2iQD7rzKck6sWnDVswkkDABS";
    CTxDestination dest = DecodeDestination(strAddress);
    auto keyid = boost::get<CKeyID>(dest);
    auto plotID = boost::get<CKeyID>(dest).GetPlotID();
    auto scriptPubKey = GetScriptForDestination(CTxDestination(keyid));

    // Simple block creation, nothing special yet:
    BOOST_CHECK(pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,CKeyID(),plotID,0,MakeTransactionRef()));
    ...
    const CAmount BLOCKSUBSIDY = 320*COIN;
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,CKeyID(),plotID,0,MakeTransactionRef()), std::runtime_error, HasReason("bad-blk-sigops"));
    mempool.clear();
    ...
    //BOOST_CHECK(pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,plotID,0,MakeTransactionRef()));
    mempool.clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,CKeyID(),plotID,0,MakeTransactionRef()), std::runtime_error, HasReason("bad-txns-inputs-missingorspent"));
    mempool.clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,CKeyID(),plotID,0,MakeTransactionRef()), std::runtime_error, HasReason("bad-cb-multiple"));
    mempool.clear();
