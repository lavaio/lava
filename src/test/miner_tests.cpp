...
static void TestPackageSelection(const CChainParams& chainparams, const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst) EXCLUSIVE_LOCKS_REQUIRED(cs_main, ::mempool.cs)
{
    ...
    std::unique_ptr<CBlockTemplate> pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,0,0,MakeTransactionRef());
    ...
    pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,0,0,MakeTransactionRef());
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,plotID,0,MakeTransactionRef()), std::runtime_error, HasReason("bad-blk-sigops"));
    mempool.clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,plotID,0,MakeTransactionRef()), std::runtime_error, HasReason("bad-txns-inputs-missingorspent"));
    mempool.clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey,0,plotID,0,MakeTransactionRef()), std::runtime_error, HasReason("bad-cb-multiple"));
    mempool.clear();
