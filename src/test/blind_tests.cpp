...
BOOST_AUTO_TEST_CASE(naive_blinding_test_from_elements)
{
    CKey key1;
    CKey key2;
    CKey keyDummy;
...

    {
        ...
        std::vector<unsigned char> value_copy(tx3.vout[0].nValueCA.vchCommitment);
        tx3.vout[0].nAsset.vchCommitment[0] = 122;
        BOOST_CHECK(!VerifyAmounts(inputs, CTransaction(tx3), nullptr, false));
        tx3.vout[0].nAsset.vchCommitment = asset_copy;
        tx3.vout[0].nValueCA.vchCommitment[0] = 122;
        BOOST_CHECK(!VerifyAmounts(inputs, CTransaction(tx3), nullptr, false));
        tx3.vout[0].nValueCA.vchCommitment = value_copy;

        // Make sure null values are handled correctly
        tx3.vout[0].nAsset.SetNull();
        BOOST_CHECK(!VerifyAmounts(inputs, CTransaction(tx3), nullptr, false));
        tx3.vout[0].nAsset.vchCommitment = asset_copy;
        tx3.vout[0].nValueCA.SetNull();
        BOOST_CHECK(!VerifyAmounts(inputs, CTransaction(tx3), nullptr, false));
        tx3.vout[0].nValueCA.vchCommitment = value_copy;
        ...
        BOOST_CHECK(VerifyAmounts(inputs, CTransaction(tx3), nullptr, false));
        tx3.vout[0].nNonce.vchCommitment = tx3.vout[0].nValueCA.vchCommitment;
        BOOST_CHECK(!VerifyAmounts(inputs, CTransaction(tx3), nullptr, false));
        ...
        BOOST_CHECK(!tx3.vout[0].nValueCA.IsExplicit());
        BOOST_CHECK(!tx3.vout[2].nValueCA.IsExplicit());
        BOOST_CHECK(VerifyAmounts(inputs, CTransaction(tx3), nullptr, false));

        CAmount unblinded_amount;
        BOOST_CHECK(UnblindConfidentialPair(key2, tx3.vout[0].nValueCA, tx3.vout[0].nAsset, tx3.vout[0].nNonce, op_true, tx3.vout[0].vchRangeproof, unblinded_amount, blind3, unblinded_id, asset_blind) == 0);
        // Saving unblinded_id and asset_blind for later since we need for input
        BOOST_CHECK(UnblindConfidentialPair(key1, tx3.vout[0].nValueCA, tx3.vout[0].nAsset, tx3.vout[0].nNonce, op_true, tx3.vout[0].vchRangeproof, unblinded_amount, blind3, unblinded_id, asset_blind) == 1);
        BOOST_CHECK(unblinded_amount == 100);
        BOOST_CHECK(unblinded_id == bitcoinID);
        CAsset temp_asset;
        uint256 temp_asset_blinder;
        BOOST_CHECK(UnblindConfidentialPair(keyDummy, tx3.vout[2].nValueCA, tx3.vout[2].nAsset, tx3.vout[2].nNonce, CScript() << OP_RETURN, tx3.vout[2].vchRangeproof, unblinded_amount, blindDummy, temp_asset, temp_asset_blinder) == 1);
        BOOST_CHECK(unblinded_amount == 0);

        // Storing for next section
        BOOST_CHECK(tx3.vout[0].nValueCA.IsCommitment());
        BOOST_CHECK(tx3.vout[0].nAsset.IsCommitment());
        blind_ozz = tx3.vout[0];

        tx3.vout[1].nValueCA = CConfidentialValue(tx3.vout[1].nValueCA.GetAmount() - 1);
        BOOST_CHECK(!VerifyAmounts(inputs, CTransaction(tx3), nullptr, false));
    }
...

    {
        ...
        BOOST_CHECK(!tx4.vout[0].nValueCA.IsExplicit());
        BOOST_CHECK(tx4.vout[1].nValueCA.IsExplicit());
        BOOST_CHECK(!tx4.vout[2].nValueCA.IsExplicit());
        // This one broken
        ...
        BOOST_CHECK(UnblindConfidentialPair(key1, tx4.vout[0].nValueCA, tx4.vout[0].nAsset, tx4.vout[0].nNonce, op_true, tx4.vout[0].vchRangeproof, unblinded_amount, blind4, asset_out, asset_blinder_out) == 0);
        BOOST_CHECK(UnblindConfidentialPair(key2, tx4.vout[0].nValueCA, tx4.vout[0].nAsset, tx4.vout[0].nNonce, op_true, tx4.vout[0].vchRangeproof, unblinded_amount, blind4, asset_out, asset_blinder_out) == 1);
        BOOST_CHECK(unblinded_amount == 30);
        BOOST_CHECK(asset_out == unblinded_id);
        BOOST_CHECK(UnblindConfidentialPair(key2, tx4.vout[2].nValueCA, tx4.vout[2].nAsset, tx4.vout[2].nNonce, op_true, tx4.vout[2].vchRangeproof, unblinded_amount, blind4, asset_out, asset_blinder_out) == 1);
        BOOST_CHECK(asset_out == unblinded_id);
        BOOST_CHECK(unblinded_amount == 50);

        // Commit to the wrong script in the rangeproof
        BOOST_CHECK(UnblindConfidentialPair(key2, tx4.vout[2].nValueCA, tx4.vout[2].nAsset, tx4.vout[2].nNonce, CScript() << OP_FALSE, tx4.vout[2].vchRangeproof, unblinded_amount, blind4, asset_out, asset_blinder_out) == 0);
        ...
        BOOST_CHECK(UnblindConfidentialPair(key2, tx4.vout[2].nValueCA, tx4.vout[2].nAsset, tx4.vout[2].nNonce, op_true, tx4.vout[2].vchRangeproof, unblinded_amount, blind4, asset_out, asset_blinder_out) == 0);
        ...
        tx4.vout[3].nValueCA = CConfidentialValue(tx4.vout[3].nValueCA.GetAmount() - 1);
        BOOST_CHECK(!VerifyAmounts(inputs, CTransaction(tx4), nullptr, false));

        // Check wallet borromean-based rangeproof results against expected args
        size_t proof_size = DEFAULT_RANGEPROOF_SIZE;
        BOOST_CHECK_EQUAL(tx4.vout[2].vchRangeproof.size(), proof_size);
        secp256k1_context *ctx = secp256k1_context_create(SECP256K1_CONTEXT_VERIFY);
        int exp = 0;
        int mantissa = 0;
        uint64_t min_value = 0;
        uint64_t max_value = 0;
        BOOST_CHECK(secp256k1_rangeproof_info(ctx, &exp, &mantissa, &min_value, &max_value, tx4.vout[2].vchRangeproof.data(), proof_size) == 1);
