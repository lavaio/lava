                    ...
                    std::vector<CCheck*>* checks, const bool store_result)
{
    ...
    if (value.IsExplicit()) {
        ...
        ret = secp256k1_pedersen_commit(secp256k1_ctx_verify_amounts, &value_commit, explicit_blinds, value.GetAmount(), &asset_gen, &secp256k1_generator_const_g);
...
bool VerifyAmounts(const std::vector<CTxOut>& inputs, const CTransaction& tx, std::vector<CCheck*>* checks, const bool store_result) {
    ...
    for (size_t i = 0; i < tx.vin.size(); ++i) {
        const CConfidentialValue& val = inputs[i].nValueCA;
        ...
        if (val.IsExplicit()) {
            ...
            if (secp256k1_pedersen_commit(secp256k1_ctx_verify_amounts, &commit, explicit_blinds, val.GetAmount(), &gen, &secp256k1_generator_const_g) != 1)
                return false;
        ...
        if (!issuance.nAmount.IsNull()) {
            if (!VerifyIssuanceAmount(commit, gen, assetID, issuance.nAmount, tx.vin[i].vchIssuanceAmountRangeproof, checks, store_result)) {
                return false;
            }
            target_generators.push_back(gen);
            vData.push_back(commit);
        ...
        if (!issuance.nInflationKeys.IsNull()) {
            ...
            if (!VerifyIssuanceAmount(commit, gen, assetTokenID, issuance.nInflationKeys, tx.vin[i].vchInflationKeysRangeproof, checks, store_result)) {
                return false;
            }
            target_generators.push_back(gen);
            vData.push_back(commit);
    ...
    {
        const CConfidentialValue& val = tx.vout[i].nValueCA;
        ...
        if (val.IsExplicit()) {
            ...
            ret = secp256k1_pedersen_commit(secp256k1_ctx_verify_amounts, &commit, explicit_blinds, val.GetAmount(), &gen, &secp256k1_generator_const_g);
            // The explicit_blinds are all 0, and the amount is not 0. So secp256k1_pedersen_commit does not fail.
            assert(ret == 1);
        }
        else if (val.IsCommitment()) {
    ...
    for(const auto& out : tx.vout) {
        const CConfidentialValue& val = out.nValueCA;
        ...
        if (QueueCheck(checks, new CRangeCheck(&val, out.vchRangeproof, vchAssetCommitment, out.scriptPubKey, store_result)) != SCRIPT_ERR_OK) {
            return false;
        }
    }
    ...
    for (const auto& out : tx.vout)
    {
        ...
        if (asset.IsExplicit()) {
            if (!out.vchSurjectionproof.empty()) {
                return false;
            }
            continue;
        }
        ...
        if (secp256k1_surjectionproof_parse(secp256k1_ctx_verify_amounts, &proof, &out.vchSurjectionproof[0], out.vchSurjectionproof.size()) != 1)
            return false;
...
bool VerifyCoinbaseAmount(const CTransaction& tx, const CAmountMap& mapFees) {
    ...
    for (const auto& out : tx.vout) {
        if (!(out.vchSurjectionproof.empty() && out.vchRangeproof.empty())) {
            return false;
        }
    }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        if (!out.nValueCA.IsExplicit() || !out.nAsset.IsExplicit()) {
            return false;
        }
        if (!MoneyRange(out.nValueCA.GetAmount())) {
            return false;
        }
        if (out.nValueCA.GetAmount() == 0 && !out.scriptPubKey.IsUnspendable()) {
            return false;
        }
        remaining[out.nAsset.GetAsset()] -= out.nValueCA.GetAmount();
    }
